...
function CreaTabla(datos){
	...
	for (var i = 0; i < datos.length; i++) {
		...
		switch(num){
			case 3:
				interes2=datos[i]['Monto']*0.05;
				interes="5%";
				break;
			case 6:
				interes2=datos[i]['Monto']*0.07;
				interes="7%";
				break;
			case 9:
				interes2=datos[i]['Monto']*0.12;
				interes="12%";
				break;
			default:
				break;
		}
		total=datos[i]['plazo']+interes2;
		...
		//Autorización
		if(datos[i]['Autorizacion']==1){
		 autorizado="Autorizado"; 
		}else{ 
			if(datos[i]['Autorizacion']==3){ 
		...
		console.log(datos[i]['Autorizacion']);
